...
void main()	{
    ...
    vec4 surface_color = vColor; //surfaceColoring*vColor+(1.0-surfaceColoring)*vec4(1.0,1.0,1.0,1.0);
...
//    diffuseLighting = diffuseLighting+evalDiffuse(p,n,0);
    ...
    if (quantizedGroups != 0.0 && quantizedGroups != 10.0) {
        lighting = round(quantizedGroups*lighting)/quantizedGroups;
    }
    // edges ???
    // if (dot(lighting, vec3(0.0,0.0,1.0)) < .001*lineWidth) {
...
//    gl_FragColor = vec4(specular, specular, specular, 1.0);
    if (vColor[3] < 0.0) // very hacky way of doing this bc I don't understand varying stuff
    {
        gl_FragColor = lineColor; // should do a color picker instead
    }
}
